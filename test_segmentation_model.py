...
def test(args, image_size=[512, 768], image_means=[0.5], image_stds=[0.5], batch_size=1):
    ...
    Seg.load_state_dict(torch.load(args.seg_ckpt_dir,weights_only=True))
...
if __name__ == "__main__":
    ...
    with open(os.path.join(args.output_dir, 'metrics.txt'), 'w') as f:
        dice=scores['dice_score'];loss=scores['avg_val_loss'];pre=scores['avg_precision']
        rec=scores['avg_recall'];fs=scores['avg_fscore']
        f.write(f'Average Dice score: {dice}')
        f.write(f'Average loss: {loss}')
        f.write('Average precision at ordered thresholds: {pre}')
        f.write('Average recall at ordered thresholds: {rec}')
        f.write('Average fscore at ordered thresholds: {fs}')